...
class _GasInjectionGraphState extends State<GasInjectionGraph> {
    ...
    return Column(
      children: [
        Container(
          ...
          color: Colors.black,
          child: Text(getLabel(widget.dataKey), style: txtStylegrey),
        ),
        SfCartesianChart(
          ...
          primaryXAxis: CategoryAxis(
            ...
            labelRotation: -70,
            labelStyle: txtStylegrey,
          ),
          primaryYAxis: NumericAxis(
            minimum: adjustedMin,
            maximum: adjustedMax,
            title: AxisTitle(
              text: getUnit(widget.dataKey),
              textStyle: txtStylegrey,
            ),
          ...
          series: <CartesianSeries<GraphRespModel, String>>[
            AreaSeries<GraphRespModel, String>(
              ...
              yValueMapper: (GraphRespModel data, _) {
                log("graphData build dataKey ${jsonEncode(data.fedGasPR.toString())}");
                return double.tryParse(getValueByKey(data, widget.dataKey)) ??
                    0;
              },
  ...
  Map<String, Map<String, String>> dataLabels = {
    'FedGas_C_Four': {'label': 'C2/C4', 'unit': 'Percentage'},
    'Plant_Load_Percentage': {'label': 'Load', 'unit': 'Percentage'},
    'FedGas_Volume': {'label': 'Inlet', 'unit': 'KSCMH'},
    'FedGas_PR': {'label': 'Flow', 'unit': 'Kg/cm²'},
    'Inlet': {'label': 'Inlet', 'unit': 'KSCMH'},
    'Flow': {'label': 'Flow', 'unit': 'Kg/cm²'},
  };
